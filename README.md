...
reports. On a refused run the same field instead holds a
machine-readable `kind` and the `error` behind it: `contended`, `broken`
or `unscoped` when the build merlint asked for did not happen, carrying
what Dune said, and `internal` when the fault is merlint's own. Every
count is zero and `passed` is false, because no lint verdict was
computed. The exit status is `4` for all four, since a caller stops and
reads the text either way; `kind` is the field it branches on.
