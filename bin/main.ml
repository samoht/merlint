      ...
      Array.fold_left
        (fun acc item ->
          ...
          try
            ...
            else if Filename.check_suffix item ".ml" then full_path :: acc
            else acc
          with _ -> acc)
        acc items
    with _ -> acc
  ...
  List.fold_left
    (fun acc path ->
      if Sys.file_exists path then
        if Sys.is_directory path then find_ml_files_in_dir path @ acc
...
let analyze_files config files =
  ...
  List.iter
    (fun file ->
      ...
      | Ok violations -> all_violations := !all_violations @ violations
      | Error msg -> Printf.eprintf "Error analyzing %s: %s\n" file msg)
    files;
...
let files =
  let doc =
    "OCaml source files or directories to analyze. If none specified, analyzes \
     all .ml files in the current dune project."
  in
...
let cmd =
  ...
  let man =
    [
      ...
      `P
        "$(tname) analyzes OCaml source files and reports functions that \
         exceed complexity or length thresholds.";
      `P
        "It uses Merlin to parse the OCaml AST and calculate cyclomatic \
         complexity based on control flow constructs.";
      `P
        "If no files or directories are specified, it analyzes all .ml files \
         in the current dune project (searching upward for dune-project).";
    ]
  in
  let info = Cmd.info "cyclomatic" ~version:"0.1.0" ~doc ~man in
  Cmd.v info
    Term.(
      const (fun complexity length files ->
          let config =
            Cyclomatic.Cyclomatic_complexity.
              { max_complexity = complexity; max_function_length = length }
          in
          if not (check_ocamlmerlin ()) then (
            ...
            else analyze_files config files_to_analyze)
