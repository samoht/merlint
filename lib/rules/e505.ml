...
let check ctx =
  match ctx with
  | Context.File _ ->
      failwith "E505 is a project-wide rule but received file context"
  | Context.Project ctx ->
      let files = Context.all_files (Project ctx) in
      (* Get executable info once for all files *)
      let executable_modules = Context.executable_modules (Project ctx) in
      let is_executable_module ml_file =
        let module_name =
          Filename.basename (Filename.remove_extension ml_file)
        in
        ...
        if is_exe then
          Logs.debug (fun m ->
              m "File %s is executable (module %s)" ml_file
