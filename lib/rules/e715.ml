...
let module_in_runner content fuzz_mod =
  let capitalized = String.capitalize_ascii fuzz_mod in
  let run_pattern =
    Re.compile (Re.seq [ Re.bow; Re.str capitalized; Re.str ".suite" ])
  in
  Re.execp run_pattern content
      ...
      try
        ...
        Log.debug (fun m ->
            m "E715: stanza '%s' has %d fuzz modules" stanza_name
              (List.length modules));
        modules
        |> List.filter (fun fuzz_mod -> not (module_in_runner content fuzz_mod))
