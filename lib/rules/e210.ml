...
let check (ctx : Context.file) =
  ...
  (* Dune wraps a virtual-library implementation as
     [Lib__lib_impl__.Submodule]: the parent library [Lib], a double
     underscore, then the implementation library name in lower-case
     with a trailing double underscore. This wrap is forced by dune
     ([implements] requires private modules) and is not under the
     author's control, so it must not trigger E210. *)
  let virtual_wrap =
    Re.compile
      (Re.seq
         [
  ...
  List.iter
    (fun (elt : Merlin.Dump.elt) ->
      ...
      let name = Merlin.Dump.name_to_string elt.Merlin.Dump.name in
      if String.contains name '_' then
        (* Skip autogenerated Dune__exe prefixes, OCaml special
           identifiers, and dune virtual-library wrap paths. *)
        if
          (not (String.starts_with ~prefix:"Dune__exe" name))
          && (not (is_special_identifier name))
          && not (Re.execp virtual_wrap name)
        then
          ...
          if Re.execp bad_pattern name then
            ...
            let clean_original =
              if String.contains name '.' then
                (* Strip known stdlib prefix if present *)
                let parts = String.split_on_char '.' name in
                match parts with
                | lib :: rest
                  when (not (Re.execp bad_pattern lib)) && List.length rest > 0
                  ->
                    String.concat "." rest
                | _ -> name
              else name
            ...
            | Some loc ->
                ...
                let loc_with_file =
                  Location.v ~file:filename
                    ~start_line:(Location.start_line loc)
                    ~start_col:(Location.start_col loc)
                    ~end_line:(Location.end_line loc)
                    ~end_col:(Location.end_col loc)
                in
                issues :=
