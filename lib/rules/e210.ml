  ...
  List.iter
    (fun (elt : Merlin.Dump.elt) ->
      ...
      if String.contains name '_' then
        (* Skip autogenerated Dune__exe prefixes, OCaml special identifiers
           (__LOC__ etc.), and identifiers whose base begins with __
           (generator-produced internals like sedlex's __sedlex_state_N or
           __private__next_int — those are not module-mangled paths). *)
        if
