...
let src = Logs.Src.create "merlint.parser" ~doc:"AST parsing"
...
let normalize_node_type (dialect : dialect) (node_type : string) : string =
  ...
  | _ -> (
      if String.length node_type < 2 then node_type
        ...
        match (prefix, dialect) with
        | 'T', Typedtree | 'P', Parsetree ->
            (* Correct prefix for the dialect, so we strip it. *)
        ...
        | 'T', Parsetree ->
            (* Special case: Tstr_attribute contains Parsetree nodes,
               so this is expected and we should strip the T prefix *)
            String.sub node_type 1 (String.length node_type - 1)
        | _ ->
...
(** Parse AST text with specific dialect (for testing) *)
let text dialect input =
  ...
  let rec transform_tree_to_ast trees =
    List.fold_left
      (fun acc tree -> merge_ast acc (process_tree tree))
      empty_acc trees
  and process_tree (Node (token, children)) =
    let raw_node_type = get_node_type token.content in
    (* Special case: str_attribute nodes contain Parsetree children *)
    let node_dialect =
      if raw_node_type = "Tstr_attribute" then Parsetree else dialect
    in
    let node_type = normalize_node_type node_dialect raw_node_type in
    Log.debug (fun m ->
        m "process_tree: raw=%s normalized=%s children=%d" raw_node_type
          node_type (List.length children));
    ...
    | "str_value" ->
        ...
        let functions = extract_functions_from_value_binding children in
    ...
    | "[" ->
        ...
        let functions = extract_functions_from_bracket_node children in
        let child_ast = transform_tree_to_ast children in
        merge_ast child_ast { empty_acc with functions }
    | "structure_item" ->
    ...
    | _ ->
        ...
        transform_tree_to_ast children
  and extract_functions_from_value_binding nodes =
      ...
      (fun acc node ->
        ...
        | Node (token, children) ->
            ...
            if node_type = "<def>" then
              match extract_function_from_def children with
              | Some func -> func :: acc
              | None -> acc
            else acc)
      [] nodes
    |> List.rev
  and extract_functions_from_bracket_node nodes =
    ...
    List.fold_left
      ...
      [] nodes
    |> List.rev
  and extract_function_from_def nodes =
    ...
    let rec find_pattern_and_expr nodes pattern_name expr_node =
      ...
      | Node (token, children) :: rest -> (
          let raw_node_type = get_node_type token.content in
          let node_type = normalize_node_type dialect raw_node_type in
          match node_type with
          | "pattern" ->
              (* Extract the name from the pattern *)
              let name = extract_name_from_pattern children in
              find_pattern_and_expr rest name expr_node
          | "expression" ->
              (* Check if this expression is a function *)
              let expr = extract_expr_if_function children in
    ...
    | _ -> None
  and extract_name_from_pattern nodes =
    ...
    List.find_map
      ...
      nodes
  and extract_expr_if_function nodes =
    ...
    let rec find_function_in_nodes nodes =
      ...
      | Node (token, _) :: rest ->
          ...
          if node_type = "exp_function" || node_type = "exp_fun" then
            ...
            Some (extract_function_expr rest)
          else find_function_in_nodes rest
    in
    find_function_in_nodes nodes
  and extract_function_expr children =
    (* Extract function expression body *)
    let body = find_function_body children in
    Log.debug (fun m ->
  ...
  and find_function_body nodes =
    ...
    | Node (token, children) :: rest -> (
        ...
        | "function_body" -> (
            ...
            match children with
            | [] -> Other
            | expr_node :: _ -> extract_expr_from_tree expr_node)
        | "exp_match" ->
            (* Match expression - siblings are the matched expr and cases *)
            extract_match_expr_from_siblings rest
        | "exp_ifthenelse" | "exp_if" ->
            (* If-then-else expression - siblings are condition, then, else *)
            extract_if_expr_from_siblings rest
        | "exp_try" ->
            (* Try expression - siblings are expr and handlers *)
            extract_try_expr_from_siblings rest
        | "exp_let" ->
            (* Let expression - siblings are bindings and body *)
            extract_let_expr_from_siblings rest
        | "expression" ->
            (* Generic expression *)
            extract_expr_from_tree (Node (token, children))
        | "exp_fun" -> (
            (* Parsetree function - last child is the body *)
            match List.rev rest with
            | body_node :: _ -> extract_expr_from_tree body_node
            | [] -> Other)
        | _ ->
            (* Try children, then siblings *)
            let child_result = find_function_body children in
            if child_result = Other then find_function_body rest
            else child_result)
  and extract_expr_from_tree (Node (token, children)) =
    ...
    | "exp_match" ->
        ...
        extract_match_expr_from_siblings children
    | "exp_ifthenelse" | "exp_if" ->
        (* Siblings contain condition, then, and else branches *)
        extract_if_expr_from_siblings children
    | "exp_try" ->
        (* Siblings contain the try expr and exception handlers *)
        extract_try_expr_from_siblings children
    | "exp_let" ->
        (* Siblings contain bindings and body *)
        extract_let_expr_from_siblings children
    | "expression" -> (
        ...
        | Node (child_token, _) :: siblings -> (
            ...
            let child_type = normalize_node_type dialect child_raw in
            match child_type with
            | "exp_ifthenelse" | "exp_if" ->
                extract_if_expr_from_siblings siblings
            | "exp_match" -> extract_match_expr_from_siblings siblings
            | "exp_try" -> extract_try_expr_from_siblings siblings
            | "exp_let" -> extract_let_expr_from_siblings siblings
            | _ -> extract_expr_from_tree (List.hd children)))
    | _ -> Other
  and extract_constant_value token children =
    ...
    if List.length children > 0 then
      ...
      | _ -> (
          ...
          | Some v -> v
          | None -> "constant")
  and extract_match_expr_from_siblings siblings =
    (* Extract match expression from sibling nodes *)
    match siblings with
    | expr_node :: rest ->
        let expr = extract_expr_from_tree expr_node in
        let case_count = count_cases rest in
        Match { expr; cases = max 1 case_count }
    | [] -> Match { expr = Other; cases = 0 }
  and extract_if_expr_from_siblings siblings =
    (* Extract if-then-else expression from sibling nodes *)
    match siblings with
    | cond_node :: then_node :: rest ->
        let cond = extract_expr_from_tree cond_node in
        let then_expr = extract_expr_from_tree then_node in
        let else_expr =
          match rest with
          | else_node :: _ -> Some (extract_expr_from_tree else_node)
          | [] -> None
        in
        If_then_else { cond; then_expr; else_expr }
    | _ -> If_then_else { cond = Other; then_expr = Other; else_expr = None }
  and extract_try_expr_from_siblings siblings =
  ...
  and extract_let_expr_from_siblings siblings =
        ...
        let rec find_bindings_and_body nodes bindings_acc =
          ...
          | Node (token, children) :: rest ->
              ...
              if node_type = "[" then
                ...
                (* This is the body *)
                (bindings_acc, extract_expr_from_tree (Node (token, children)))
...
let typedtree input =
  ...
  with Type_error ->
    (* Type errors in the code - try with Parsetree dialect *)
