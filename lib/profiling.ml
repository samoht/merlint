  ...
  let rows =
    List.map
      (fun (file, merlin, rules, count) ->
        [ file; ms merlin; ms rules; string_of_int count; ms (merlin +. rules) ])
      top
  in
  let rows =
    if remaining > 0 then
...
let print_file_summary t =
  ...
  if timings = [] then ()
    ...
    if file_stats = [] then ()
      ...
      in
      let rows = format_file_rows sorted in
    ...
    let rows =
      List.map
        (fun (code, count, total, is_project) ->
          let avg = Fmt.str "%.1f" (total *. 1000.0 /. float_of_int count) in
          [
            code;
...
let print_rule_summary t =
  ...
  if timings = [] then ()
    ...
    if rule_stats = [] then ()
      ...
      in
      let rows = format_rule_rows sorted in
      if rows = [] then ()
      else
        let columns =
          Tty.Table.
