...
let run_dune_describe project_root =
  let cmd = Printf.sprintf "cd %s && dune describe" (Filename.quote project_root) in
  ...
  | Unix.WEXITED 0 -> 
      ...
      Log.debug (fun m -> m "Dune describe successful, output length: %d" (String.length sexp_str));
      Ok sexp_str
  | Unix.WEXITED 127 ->
...
let ensure_project_built project_root =
  ...
  if not (Sys.file_exists build_dir) then (
    ...
    let cmd = Printf.sprintf "cd %s && dune build" (Filename.quote project_root) in
