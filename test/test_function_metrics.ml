...
let test_empty_structure_has_no_values () =
  Alcotest.(check int)
    "no values" 0
    (List.length (Merlint.Function_metrics.of_structure empty_structure))

let test_empty_view_exposes_no_metrics () =
  let view =
...
let tests =
  [
    ("empty_structure_has_no_values", `Quick, test_empty_structure_has_no_values);
    ("empty_view_exposes_no_metrics", `Quick, test_empty_view_exposes_no_metrics);
  ]
