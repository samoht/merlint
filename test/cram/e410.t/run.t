...
which would report the run incomplete if a rule had wanted a typedtree:
  ...
  $ merlint --build -r E410 bad.mli > built.txt
  [1]
  $ merlint --build -r E410 good.mli > built-good.txt
  $ rm -rf _build
  $ find . -name '*.cmt*'
  $ merlint -r E410 bad.mli > unbuilt.txt
  [1]
  $ merlint -r E410 good.mli > unbuilt-good.txt
