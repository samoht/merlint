...
rest are:
  ...
  ! 12 files are claimed by no dune stanza, so nothing compiles them and no rule examined them. Three ways that happens: no stanza names them (a [(modules ...)] spec may be excluding them); they no longer belong in the tree; or a stanza does name them and merlint's project index could not read that stanza, which is a defect in merlint and not one of yours. Check which with [project-index stanzas -p <pkg>] and [project-index libraries -p <pkg>]: a stanza that is in the dune file and in neither listing is the third.
  lib/orphan_01.ml
  lib/orphan_02.ml
  lib/orphan_03.ml
  lib/orphan_04.ml
