...
A qualified allowed_words entry (Allowed.download_to_path) exempts only that
binding; path_to_uri is still flagged:
  $ merlint --build -r E333 allowed.ml
  Dune root: $TESTCASE_ROOT/
  Running merlint analysis...
  ...
  ✗ Naming Conventions (1 total issues)
    [E333] Prefer _of_ over _to_ / _from_ (1 issue)
    Standalone conversion functions in OCaml use the [<dst>_of_<src>] form
    ([int_of_string], [string_of_float], ...) — never [<src>_to_<dst>],
    [<dst>_from_<src>], [<X>_of_t], or [t_of_<X>]. The convention reads as 'an X
    out of a Y' and matches the stdlib precedent. The [to_<X>] prefix is reserved
    ...
    functions ([print_to_buffer : Buffer.t -> string -> unit]) are skipped by
    their type. Add domain-specific exceptions to [allowed_words].
    - allowed.ml:1:0: Function 'path_to_uri' uses a non-canonical conversion-naming form; OCaml convention is 'uri_of_path' (the [<dst>_of_<src>] form, matching [int_of_string], [string_of_float], ...).
  ✓ Documentation (0 total issues)
  ✓ Project Structure (0 total issues)
  ✓ Test Quality (0 total issues)
  ✓ Interop Testing (0 total issues)
  ...
  │ Category           │ Issues                               │
  ├────────────────────┼──────────────────────────────────────┤
  │ Naming Conventions │ 1 (1 prefer _of_ over _to_ / _from_) │
  ╰────────────────────┴──────────────────────────────────────╯
