Test bad example - should flag the standalone _to_ conversions but not action-verb names:
  ...
  ✗ Naming Conventions (5 total issues)
    ...
    stdlib precedent. Module-method conversions ([Bytes.to_string],
    [Bytes.of_string]) are unaffected: the rule only flags identifiers that
    contain [_to_] / [_from_] inside the name itself, and skips action-verb
    prefixes ([add_to_set], [walk_to_root], [print_to_buffer], [read_from_buffer],
    [recover_from_error], etc.). Add domain-specific exceptions to
    [allowed_words].
    ...
    - bad.ml:14:0: Function 'value_from_string' uses the [<src>_to_<dst>] / [<dst>_from_<src>] form; OCaml convention is 'value_of_string' (the [<dst>_of_<src>] form, matching [int_of_string], [string_of_float], ...)
    - bad.ml:15:0: Function 'bytes_from_hex' uses the [<src>_to_<dst>] / [<dst>_from_<src>] form; OCaml convention is 'bytes_of_hex' (the [<dst>_of_<src>] form, matching [int_of_string], [string_of_float], ...)
