...
let test_relative_outside_root_keeps_original () =
  let root = Fpath.v "/workspace/project" in
...
let test_in_file_uses_fpath_string () =
...
let tests =
  [
    ...
    ("relative_outside_root", `Quick, test_relative_outside_root_keeps_original);
    ("in_file_uses_fpath_string", `Quick, test_in_file_uses_fpath_string);
  ]
