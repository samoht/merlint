...
let test_extract_location_from_parsetree () =
...
let test_extract_filename_from_parsetree () =
  let text = "Pexp_ident \"Obj.magic\" (bad_style.ml[2,27+16]..[2,27+25])" in
...
let test_check_obj_magic () =
  ...
  | [ Violation.No_obj_magic { location = { file; line; col } } ] ->
      ...
      Alcotest.check Alcotest.int "line" 2 line;
      Alcotest.check Alcotest.int "col" 16 col
  | _ -> Alcotest.fail "Expected 1 Obj.magic violation"
...
let test_full_parsetree_sample () =
  ...
  in
  let violations = Style_rules.check (`String sample_text) in
  Alcotest.check Alcotest.bool "has violations" true (List.length violations > 0)

let tests =
  [
    Alcotest.test_case "location_extraction" `Quick
      test_extract_location_from_parsetree;
    Alcotest.test_case "filename_extraction" `Quick
      test_extract_filename_from_parsetree;
